...
## Dev Container内のClaude権限設定

このプロジェクトはDev Container内で動作するため、以下の権限設定が適用されます。

### 自動承認される操作（ユーザーの承認待ちなし）
...
#### コマンド実行
- ✅ Bashコマンド実行（Dev Container内）
- ✅ テスト実行、ビルド実行、パッケージインストール

#### Git操作
- ✅ ローカルGit操作（add、commit、branch作成）
- ✅ **条件付きpush**: プライベートリポジトリ、feature/XXXブランチ、通常push

#### GitHub操作
- ✅ **条件付き**: プライベートリポジトリでのPR/Issue作成
...
- ⚠️ パブリックリポジトリへの操作
- ⚠️ データベース変更、外部API呼び出し
- ⚠️ ユーザーへの質問や重要な報告

### 理由

Dev Container内は隔離された環境であり、全ての変更はGit管理されています。
開発効率を優先しつつ、外部への影響がある操作については安全性を確保します。

詳細は `~/projects/claude-dev-standards/README.md` の「Dev Container内のClaude権限設定」セクションを参照してください。
